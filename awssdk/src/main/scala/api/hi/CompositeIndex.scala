...
private[meteor] sealed abstract class CompositeIndex[
  ...
  /** Retrieve items from a composite index, can be a secondary index or a table
    * which has composite keys (partition key and sort key).
    *
    * @param query
...
/** Represent a secondary index where the index has composite keys (partition
  * key and sort key).
  *
  * @param tableName
  *   table's name
...
case class SecondaryCompositeIndex[F[_]: Async, P: Encoder, S: Encoder](
  ...
  /** Retrieve all items with the same partition key as a fs2 Stream. The number
    * of returned items can be limited. A Stream is returned instead of a list
    * because the number of returned items can be very large. This always
    * performs eventually consistent reads as strong consistent is not supported
    * for secondary index.
    *
    * @param partitionKey
    *   partition key
...
/** Represent a table where the index is composite keys (partition key and sort
  * key).
  *
  * @param tableName
  *   table's name
...
case class CompositeTable[F[_]: Async, P: Encoder, S: Encoder](
  ...
  /** Update an item by composite keys given an update expression when a
  ...
  /** Retrieve all items with the same partition key as a fs2 Stream. The number
    * of returned items can be limited. A Stream is returned instead of a list
    * because the number of returned items can be very large.
    *
    * @param partitionKey
    *   partition key
    * @param consistentRead
  ...
  /** Get items by composite keys in batch. Max batch size is preset to 100
  ...
  /** Put items in batch, '''in ordered'''. Meaning batches are processed in
    ...
    * composite keys. If your input doesn't have this constrain, you can use
  ...
  /** Put items in batch, '''un-ordered'''. Meaning batches are processed in
    * '''parallel''', hence, if your input has items with the same composite
    * keys, this can cause a race condition, consider using [[batchPut]]
    * instead. Max batch size is preset to 25 (maximum batch size permitted from
    * batch put actions), however, it is possible to control the rate of
    * batching with `maxBatchWait` parameter. This uses fs2 .groupWithin
    * internally. This returns a Pipe in order to cover broader use cases
    * regardless of input can be fitted into memory or not. Duplicated items
    * within a batch will be removed. Left over items within a batch will be
    * reprocessed in the next batch.
  ...
  /** Delete items by composite keys in batch. Max batch size is preset to 100
    * (maximum batch size permitted for batch get action), however, it is
    * possible to control the rate of batching with `maxBatchWait` parameter.
    * This uses fs2 .groupWithin internally. This returns a Pipe in order to
    * cover broader use cases regardless of input can be fitted into memory or
  ...
  /** Write items (put or delete) in batch, '''in ordered'''. Meaning batches
    ...
    * with the same composite keys. Max batch size is preset to 25 (maximum
    * batch size permitted from batch write actions), however, it is possible to
    ...
    *   returned item's type
    * @return
    *   a fs2 Pipe from Either[(P, S), T], represent deletion (Left) or put
    *   (Right) to Unit.
    */
